...
%possible additions
\item $N_i$ is a \emph{put} node, $N_j$ is a \emph{reachable} \emph{get} node and $N_i \rightarrow N_j$ is in $G$. %first metion of G?
...
%The only way for a start node to become reachable is for n to already be in comps.
...
enable any disabled flow-edges into the get. %But do not mark the get reachable at this time.
...
\item When marking a get node, give it the same label as the put node leading to it. % What if the put isn't marked? See Ex12 cycleA.
...
\item For each call edge, $N_i \rightarrow N_j$, in the cycle,
...
\item For each call edge, $N_i \rightarrow N_j$, in the cycle, add the label for all call nodes reachable from $N_i$ following
only flow edges to $calls$ and add the start node of the call to $candidates$.
\item While there exists a label ${n} \in candidates$ such that ${n} \in comps$ remove ${n}$ from $candidates$
and all all flow-edge reacahble frm ${n}$ call node labels to $calls$.

%Need to give call and start nodes the same label.
...
\end{itemize}
\end{shaded}
\caption{\label{call-cons-analysisxx}
Algorithm to compute the call and comp sets for a cycle.}
\end{figure}
}
